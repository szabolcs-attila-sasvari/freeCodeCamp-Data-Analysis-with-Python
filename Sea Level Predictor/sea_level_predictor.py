...
def draw_plot():
  ...
  plt.plot(first_extended,
           [first_reg.intercept + first_reg.slope * x for x in first_extended],
           'r')

  # Create second line of best fit
  second_extended = np.arange(2000, 2051, 1)
  twentieth_century = df[2000 <= df['Year']]
  second_reg = linregress(twentieth_century['Year'],
                          twentieth_century['CSIRO Adjusted Sea Level'])
  plt.plot(
    second_extended,
    [second_reg.intercept + second_reg.slope * x
     for x in second_extended], 'g')
